...
extension NSDate {
	...
	@nonobjc static let JulianEpoch: NSTimeInterval = 2440588 /// The Julian representation of the Unix Epoch
	...
	var julianRepresentation: NSTimeInterval { /// Returns the Julian representation of `self`
		return timeIntervalSince1970 / NSDate.DayInMilliseconds - 0.5 + NSDate.JulianEpoch
	}
	...
	convenience init(julianDate: NSTimeInterval) { /// Initializes a proper NSDate from its Julian representation
		self.init(timeIntervalSince1970: (julianDate + 0.5 - NSDate.JulianEpoch) * NSDate.DayInMilliseconds)	
	}
